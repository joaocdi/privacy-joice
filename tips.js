...
(() => {
  ...
  const save=()=>{try{localStorage.setItem(config.productId+'.pending',JSON.stringify(state));}catch(_){}};
  ...
  async function open(){
    ...
    try{
      const r=await fetch('/api/tips/config',{signal:AbortSignal.timeout(10000)});if(!r.ok)throw Error('Mimos indisponíveis agora.');config=await r.json();if(!config.available)throw Error('Mimos indisponíveis neste meio de pagamento.');
      if(!modal.open)return;
      if(!state){try{const old=JSON.parse(localStorage.getItem(config.productId+'.pending'));if(old&&/^[a-f0-9]{64}$/.test(old.token)&&Number.isSafeInteger(old.cents)&&old.cents>=config.minCents&&old.cents<=config.maxCents)state=old;}catch(_){}}
      say('');if(state){$('tipAmount').value=(state.cents/100).toFixed(2).replace('.',',');locked();if(state.payment?.pix)showPix();else{$('tipGenerate').textContent='RETOMAR ESTE PIX';say('Retome o mesmo pedido para evitar cobrança duplicada.');}}
  ...
  $('tipCopy').onclick=async()=>{try{await navigator.clipboard.writeText($('tipCode').value);$('tipCopy').textContent='CÓDIGO COPIADO';setTimeout(()=>{$('tipCopy').textContent='COPIAR CÓDIGO PIX';},2000);}catch(_){$('tipCode').focus();$('tipCode').select();say('Selecione e copie o código acima.');}};
