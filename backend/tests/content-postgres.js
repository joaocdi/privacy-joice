...
async function main() {
  ...
  assert.deepEqual((await require('../services/profile').get()).stats, {posts:1,photos:1,videos:0,likes:0});
  ...
  assert.equal((await require('../services/profile').get()).stats.likes, 1);
  ...
  assert.equal((await require('../services/profile').get()).stats.likes, 1, 'manual post counters do not inflate profile likes');
