...
module.exports = async function testProfile(db) {
  ...
  assert.equal(initial.name, 'Maya');
  ...
  assert.equal(renamed.name, 'Maya');
  assert.equal(renamed.username, '@_maya.vl');
  assert.equal(renamed.bio, 'Oi Maya');
  const body = { ...renamed, name: 'Maya QA', bio: '<script>texto como texto</script>' };
